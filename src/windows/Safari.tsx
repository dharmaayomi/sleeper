  ...
  return (
    <section ref={containerRef} id="safari" className="window">
      ...
      {isMobile ? (
        <div ref={headerRef} id="window-header" className="relative flex items-center justify-between select-none">
          ...
          <div className="search flex items-center gap-1.5 bg-neutral-100 border border-neutral-200 rounded-lg px-2.5 py-1 w-[55%] truncate mr-2 select-none">
            ...
            <span className="text-[11px] text-gray-500 font-inter truncate">articles.com</span>
          </div>
        </div>
      ) : (
      ...
      <div className="blog h-[calc(100%-2.75rem)] overflow-y-auto p-6 pb-16 max-w-3xl mx-auto">
        <h2 className="text-xl font-inter font-bold text-pink-600 mb-8">My Blog</h2>

        <div className="space-y-8">
          {blogPosts.map((post) => (
            <div
              key={post.id}
              className={isMobile ? "flex flex-col gap-3" : "blog-post grid grid-cols-12 space-x-5"}
            >
              <div className={isMobile ? "w-full rounded-lg overflow-hidden" : "col-span-2"}>
                <img
                  src={post.image}
                  alt={post.title}
              ...
              <div className={`content ${isMobile ? "space-y-1.5" : "col-span-10 space-y-3"}`}>
                <p className="text-[11px] font-inter text-gray-400">{post.date}</p>
